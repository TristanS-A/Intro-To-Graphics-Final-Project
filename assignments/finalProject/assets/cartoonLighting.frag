...
void main(){
    ...
    for (int i = 0; i < MAX_LIGHTS; i++){
        ...
        lightColor += newColor * _Mat.ambientK; //Ambient calculations part
        ...
        specularLighting = _Mat.specular * pow(max(dot(newNormals, normalize(halfVecPart / length(halfVecPart))), 0), _Mat.shininess) * newRange;

        //Branch to toggle extra light with ImGui
        if (_ExtraLight) {
            float tempLightColor = step(0.5, rimLighting);
            tempLightColor += step(0.35, rimLighting);

            lightColor += (newColor * 0.5) * tempLightColor;

            tempLightColor = step(0.5, specularLighting);
            tempLightColor += step(0.3, specularLighting);
